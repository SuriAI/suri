...
import type { GroupWithMembers, EditingMember, EditingGroup, MemberField, GroupField } from "../types";
...
export function useDatabaseManagement(
    ...
    const filteredData = useMemo(() => {
        ...
        return groupsWithMembers
            ...
            .filter(
                (group) => group.members.length > 0 || group.name.toLowerCase().includes(query)
            );
    ...
    const saveEdit = useCallback(async (personId: string, field: MemberField, value: string) => {
    ...
    const saveGroupEdit = useCallback(async (groupId: string, field: GroupField, value: string) => {
    ...
    const handleDeleteGroup = useCallback(async (groupId: string, groupName: string) => {
        ...
        try {
            ...
            if (success) {
                try {
                    const { useGroupStore } = await import("../../../group/stores/index");
    ...
    const handleDeleteMember = useCallback(async (personId: string, memberName: string) => {
