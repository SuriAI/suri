            ...
            {results.length > 0 && (
                <div className="space-y-2 max-h-64 overflow-y-auto">
                    {results.map((result, idx) => (
                        <div
                            key={idx}
                            className={`rounded-xl border p-3 flex items-start gap-3 ${result.success
                                    ? "border-cyan-400/20 bg-cyan-500/5"
                                    : "border-red-400/20 bg-red-500/5"
                                }`}
                        >
                            <div
                                className={`h-6 w-6 rounded-lg flex items-center justify-center text-sm ${result.success ? "bg-cyan-500/20" : "bg-red-500/20"
                                    }`}
                            >
                                {result.success ? "✓" : "✕"}
                            </div>
