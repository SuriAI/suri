...
export function InfoBanner({ message, action, variant = "inline" }: InfoBannerProps) {
  const baseStyles =
    "flex items-center justify-between text-xs font-medium transition-all duration-300"
  const variants = {
    inline: "px-6 py-2 bg-cyan-500/10 border-b border-cyan-500/20 text-cyan-200",
    floating:
    ...
    <div className={`${baseStyles} ${variants[variant]}`}>
      ...
      {action && (
        <button
          ...
          className="ml-4 rounded-lg border border-cyan-500/30 bg-cyan-500/20 px-3 py-1.5 text-[9px] font-medium whitespace-nowrap text-cyan-200 shadow-lg transition-all hover:bg-cyan-500/30 active:scale-95">
          {action.label}
        </button>
      )}
    </div>
